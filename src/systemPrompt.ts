...
- 太字は *テキスト* と記述します（単一のアスタリスクの前後にスペースが必要）
- 斜体は _テキスト_ と記述します（アンダースコアの前後にスペースが必要）
- 取り消し線は ~テキスト~ と記述します（単一のチルダの前後にスペースが必要）
- コードブロックは \`\`\`言語名\n コード \`\`\` と記述します
- インラインコードは \`コード\` と記述します
- 引用は >テキスト と記述します

標準的なマークダウン記法（**太字**、*斜体*、~~取り消し線~~など）ではなく、必ずSlack形式のマークダウンを使用してください。また、太字・斜体・取り消し線の記号の前後にはスペースを入れてください。例えば「太字のために *アスタリスク* を使った」のように記述します。`;
